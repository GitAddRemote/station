...
describe('OrbitsSyncStep', () => {
  ...
  describe('syncOrbits', () => {
    it('upserts each orbit with a known star system', async () => {
      uexGet.mockResolvedValueOnce([makeOrbit({ id: 1, id_star_system: 10 })]);
    ...
    it('skips and warns when orbit has no name', async () => {
      uexGet.mockResolvedValueOnce([makeOrbit({ id: 3, name: '' })]);
    ...
    it('skips and warns when orbit references unknown star system', async () => {
      uexGet.mockResolvedValueOnce([makeOrbit({ id: 4, id_star_system: 999 })]);
    ...
    it('skips and warns when id_star_system is null', async () => {
      uexGet.mockResolvedValueOnce([
        makeOrbit({ id: 5, id_star_system: null }),
      ]);
    ...
    it('emits top-level warning when station_star_system is empty', async () => {
      uexGet.mockResolvedValueOnce([makeOrbit({ id: 6, id_star_system: 10 })]);
