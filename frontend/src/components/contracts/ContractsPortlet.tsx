...
const ContractsPortlet = ({ size }: ContractsPortletProps) => {
  ...
  const [statusFilter, setStatusFilter] = useState<ContractStatus | 'all'>('open');
  ...
  useEffect(() => {
    setLoading(true);
    const params: Record<string, string | number> = { limit: 5, page: 1 };
    if (statusFilter !== 'all') params.status = statusFilter;
    api
      .get<PaginatedContracts>('/api/contracts', { params })
      .then((res) => {
  ...
  const emptyLabel = statusFilter === 'all' ? 'No contracts yet.' : `No ${STATUS_LABEL[statusFilter as ContractStatus]?.toLowerCase() ?? statusFilter} contracts.`;
    ...
    <>
      <div className="cp-filter-bar">
        {FILTER_OPTIONS.map((opt) => (
          <button
            key={opt.value}
            className={'cp-filter-pill' + (statusFilter === opt.value ? ' active' : '')}
      ...
      {loading ? (
        ...
        <>
            ...
            <table className="inv-table">
              <thead>
                <tr>
                  ...
                  <th>Status</th>
                  <th>Ends</th>
                  {isStandardOrFull && <th>Claimed By</th>}
          ...
          <div className="p-pagination">
            <span className="p-count">{Math.min(5, total)} of {total.toLocaleString()}</span>
          </div>
        </>
      )}
    </>
