...
export default function CreateContractModal({ onClose, onCreated, initialType = 'transfer', initialTitle = '' }: CreateContractModalProps) {
  ...
  const handleSubmit = useCallback(async (e: React.FormEvent) => {
    ...
    setSaving(true);
    try {
      const res = await api.post<{ id: string }>('/api/contracts', {
        title,
        type,
        orgId,
      ...
      // leave modal open on error; backend validation surfaces as 400/422
    } finally {
      setSaving(false);
    }
  }, [title, type, orgId, donation, rewardAuec, deadline, description, assigneeKind, assigneeUserId, assigneeOrgId, onCreated, onClose]);
